 ...
 */
    ...
    Pos
    getFeaturePos(StructureConfig config, uint64_t seed, int regX, int regZ);

    static inline __attribute__((const))
    Pos
    getFeatureChunkInRegion(StructureConfig config, uint64_t seed, int regX, int regZ);

    static inline __attribute__((const))
    Pos
    getLargeStructurePos(StructureConfig config, uint64_t seed, int regX, int regZ);

    static inline __attribute__((const))
    Pos
    getLargeStructureChunkInRegion(StructureConfig config, uint64_t seed, int regX, int regZ);
 ...
 */
    ...
    static inline __attribute__((const)) int isSlimeChunk(uint64_t seed, int chunkX, int chunkZ)
    {
        uint64_t rnd = seed;
 ...
 */
    static inline __attribute__((always_inline, const)) float isQuadBaseFeature24Classic(const StructureConfig sconf, uint64_t seed);

    static inline __attribute__((always_inline, const)) float isQuadBaseFeature24(const StructureConfig sconf, uint64_t seed,
                                                                                  int ax, int ay, int az);

    static inline __attribute__((always_inline, const)) float isQuadBaseFeature(const StructureConfig sconf, uint64_t seed,
                                                                                int ax, int ay, int az, int radius);

    static inline __attribute__((always_inline, const)) float isQuadBaseLarge(const StructureConfig sconf, uint64_t seed,
                                                                              int ax, int ay, int az, int radius);
 ...
 */
    __attribute__((deprecated)) int findStrongholds(
        const int mcversion,
        const LayerStack *g,
        int *cache,
        Pos *locations,
 ...
 */
    inline static uint64_t chunkGenerateRnd(uint64_t worldSeed, int chunkX, int chunkZ)
    {
        uint64_t rnd;
 ...
 */
    ...
    */
    ...
    static __attribute__((const)) float getEnclosingRadius(
                                                                                  ...
                                                                                  int ax, int ay, int az)
    {
        ...
        s00 ^= K;
        JAVA_NEXT_INT24(s00, x0);
        if L (x0 < 20)
            return 0;
        JAVA_NEXT_INT24(s00, z0);
        if L (z0 < 20)
            return 0;

        s11 ^= K;
        JAVA_NEXT_INT24(s11, x1);
        if L (x1 > x0 - 20)
            return 0;
        JAVA_NEXT_INT24(s11, z1);
        if L (z1 > z0 - 20)
            return 0;
        ...
        s01 ^= K;
        JAVA_NEXT_INT24(s01, x2);
        if L (x2 >= 4)
            return 0;
        JAVA_NEXT_INT24(s01, z2);
        if L (z2 < 20)
            return 0;

        s10 ^= K;
        JAVA_NEXT_INT24(s10, x3);
        if L (x3 < 20)
            return 0;
        JAVA_NEXT_INT24(s10, z3);
        if L (z3 >= 4)
            return 0;
